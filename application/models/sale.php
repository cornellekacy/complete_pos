...
class Sale extends CI_Model
{
	...
	function save ($items,$customer_id,$employee_id,$comment,$payments,$sale_id=false)
	{
		...
		foreach($payments as $payment_id=>$payment)
		{
            ...
            // add poin if payment >25K
            $purchase_point = to_number($this->config->item('purchase_point'));
            if(($payment['payment_amount']>=$purchase_point)and($customer_id!=-1)){
                $current_point = $this->Customer->get_info($customer_id);
                $current_point=$current_point->point;
                
                $add_point = floor($payment['payment_amount'] / $purchase_point);
                
                $this->db->update('customers',array('point'=>($current_point+$add_point)),array('person_id'=>$customer_id ),1);
            }
		}
