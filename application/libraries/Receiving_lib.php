...
class Receiving_lib
{
	...
	{
		if( !$this->CI->session->userdata( 'receive_payments' ) )
			$this->set_payments( array( ) );

		return $this->CI->session->userdata('receive_payments');
	}

	//Alain Multiple Payments
	function set_payments($payments_data)
	{
		$this->CI->session->set_userdata('receive_payments',$payments_data);
	}
    
    //Alain Multiple Payments
	function empty_payments()
	{
		$this->CI->session->unset_userdata('receive_payments');
	}
	...
	function get_amount_due()
	{
		...
		$amount_due=($payment_total - $sales_total);
		return $amount_due;
	}
